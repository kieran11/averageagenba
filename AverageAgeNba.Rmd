---
title: "Average Length of NBA Careers"
...
The first step is to pull all player data for these years. We use the function `NBAPerGameStatisticsPer36Min`, and only keep the distinct player names and links. In the dataset, there is a variable called `link`, which provides the link to that player on Basketball Reference. 
...
AllPlayers = dget("CompleteDataSet") %>% 
  filter( 
    !is.na(age) & tm != "TOT" 
    ) %>% 
  group_by(link) %>% 
